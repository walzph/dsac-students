...
point_nn = CircleNN(opt.capacity, opt.receptivefield)
if opt.gpu: point_nn = point_nn.cuda()
point_nn.train()
...
direct_nn = CircleNN(opt.capacity, 0, True)
if opt.gpu: direct_nn = direct_nn.cuda()
direct_nn.train()
...
val_images, val_labels = dataset.sample_circles(opt.valsize)
...
for iteration in range(0, opt.trainiterations+1):
	...
	inputs, labels = dataset.sample_circles(opt.batchsize)
